...
TEST_DATA_DIR = r"./ind_homes"  # Directory for independent home data
...
def test(test_file_path, columns):
    """
    Test preprocessing on a single file to identify issues causing infinite NRMSE.

    Args:
    test_file_path (str): Path to the test file.
    columns (list): List of columns to check, e.g., ['elec', 'gas'].

    Returns:
    dict: A dictionary with detected issues or 'Pass' if no issues are found.
    """
    issues = {}
    ...
    for col in columns:
        if col not in processed_df.columns:
            issues[col] = f"Missing column '{col}' after preprocessing."
            continue  # Skip further checks for this column

        # Check for NaN values
        if processed_df[col].isna().sum() > 0:
            issues[col] = f"Column '{col}' contains NaN values."

        # Check if column values are all zeros or constant
        if processed_df[col].nunique() == 1:
            issues[col] = (
                f"Column '{col}' contains constant values ({processed_df[col].iloc[0]})."
            )

        # Check if column mean is 0
        col_mean = processed_df[col].mean()
        if col_mean == 0:
            issues[col] = f"Column '{col}' has a mean value of 0."

        # Check summary statistics for anomalies
        col_summary = processed_df[col].describe()
        if col_summary["min"] < 0:
            issues[col] = f"Column '{col}' contains negative values."

    # Check overall DataFrame for timestamp alignment or excess NaN rows
    if processed_df.isna().any().any():
        issues["overall"] = "Data contains NaN rows that are not fully cleaned."

    return issues if issues else "Pass"


def debug_file_processing(test_file_path):
    """
    Debug the processing of a single problematic file, inspecting raw and preprocessed data.
    """
    # Load raw data
    raw_df = pd.read_csv(test_file_path)
    print(f"\nRaw data summary for {test_file_path}:\n")
    # print(raw_df.info())
    ...
    print(f"\nPreprocessed data summary for {test_file_path}:\n")
...
if __name__ == "__main__":
    ...
    debug_file_processing(os.path.join(TEST_DATA_DIR, "home_home304.csv"))
